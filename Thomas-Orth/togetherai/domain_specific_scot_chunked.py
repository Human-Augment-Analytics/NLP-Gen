...
def generate_prompt(document):
    prompt = f"""
      ...
      7. Is this a class action lawsuit?
      ...
      8. Any elements of the case that add value, such as notable conflicts, events, or significant turning points in the litigation.
      9. Any events or detail not covered by the requested entities.
      Using the extracted entities, also provide a summary about these entities.
    """
    return prompt
...
def generate_prompt_combined(chunks):
    prompt = f"""
      ...
      1. Start with a lede sentence to draw in the reader. If it was a class action case, include that in the lede. If it is not a class action, don't mention that
      2. Next, describe the facts of case
      3. Mention the important details such as:
          * Filing Date
          * Full name of the court where the case was filed
          * The name and title of the Judge for this case, if present. If not present, do not mention it.
          ...
          * The rememdies sought by the plaintifs, providing details and information about the declaratory relief, injuctive relief, attorney fees or money fees requested in detail as provided.
          * Any elements of the case that add value, such as notable conflicts, events, or significant turning points in the litigation.
      ...
      6. Do not end the summary highlighting the impact ofthe case.
      7. For the summary, use a professional tone and present in a way that someone without a legal background can understand
...
csv_data = []
llm = ChatTogether(model=model_name, temperature=0.0000000001)
...
df_save.to_csv(f"june_deliverable/generated_results_{model_name}_chunked_{str(uuid.uuid4())}.csv")
"""